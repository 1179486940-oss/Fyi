...
## 11. 当前版本限制（v2 增强: 务实文档）

> 以下限制清单来自 v2 `说明文档.md` 第 13 章，经 v1 生产环境适配后更新。

### 11.1 已完成项 ✅

| # | 项目 | 状态 |
|---|------|------|
| 1 | 统一数据模型层 (`core/models.py` — 9 个核心数据结构) | ✅ v2 增强 |
| 2 | 独立确认服务 (`core/confirmation_service.py`) | ✅ v2 增强 |
| 3 | 独立查询服务 (`core/query_service.py` — QueryPlan 抽象) | ✅ v2 增强 |
| 4 | 测试覆盖 (`tests/test_router.py` + `tests/test_query_agent.py`) | ✅ v2 增强 |
| 5 | 工具函数增强 (`utils/helpers.py` + `utils/logger.py`) | ✅ v2 增强 |
| 6 | Per-Agent 子目录结构 (`skills/` + `mcp/` 独立子包) | ✅ v2 增强 |
| 7 | 独立 artifact 目录 (`artifacts/`) | ✅ v2 增强 |
| 8 | 文档务实化（限制清单 + 迭代方向） | ✅ v2 增强 |

### 11.2 待完成项 🔧

| # | 限制 | 影响范围 | 优先级 | @REAL_CODE 位置 |
|---|------|---------|--------|-----------------|
| 1 | QueryService 的 Text2SQL 仍是 Mock 实现 | 数据查询 Agent | **HIGH** | `core/query_service.py:build_plan()` |
| 2 | 写操作确认只做到"挂起返回确认信息"，确认后继续执行的完整闭环待补全 | 所有写操作 | **HIGH** | `core/confirmation_service.py` |
| 3 | Graph / Report / PPT Agent 目前未真正消费 Query Agent 的输出数据 | 多意图链路 | **HIGH** | 各子 Agent 的 `process()` |
| 4 | 产物文件存储仍为本地目录，未对接 S3/MinIO | 文件下载 | **HIGH** | `core/models.py:Artifact` |
| 5 | 路由意图识别基于 Embedding + LLM，但特定场景仍需微调阈值 | 路由准确率 | MEDIUM | `core/guardrails.py:_compute_dynamic_threshold()` |
| 6 | 写操作检测基于关键词匹配，未升级为 LLM 语义判断 | 路由准确率 | MEDIUM | `core/guardrails.py:detect_write_intent()` |
| 7 | 多意图链路目前是并行执行，未支持 DAG 依赖调度 | 复杂任务 | MEDIUM | `agents/router_agent.py:_multi_split_node()` |
| 8 | 确认超时后的自动清理机制未实现 | 内存泄漏 | MEDIUM | `core/confirmation_service.py:cleanup_expired()` |
| 9 | Report Version 模糊匹配未对接真实数据库查询 | 图表 Agent | MEDIUM | `agents/sub_agents/data_graph_agent.py` |
| 10 | 测试依赖全局单例，未使用 pytest fixtures 注入 mock | 测试可维护性 | MEDIUM | `tests/test_router.py` |
| 11 | 确认状态存储于进程内存，多实例部署需迁移到 Redis | 分布式部署 | LOW | `core/confirmation_service.py:_states` |
| 12 | PPT 模板匹配基于关键词，未对接 BA 真实模板服务 | PPT Agent | LOW | `agents/sub_agents/ppt_generate_agent.py` |

---

## 12. 后续迭代方向（v2 增强: 务实路线图）

> 以下迭代路线图来自 v2 `说明文档.md` 第 14 章，经 v1 生产环境重新排定优先级。

### Phase 1: 核心链路真实化（当前阶段）

1. **Text2SQL 对接真实 LLM + DB**
   - 替换 `QueryService.build_plan()` 中的 Mock 分支
   - 对接 RAGFlow 数据库KB → 获取表 DDL → LLM 生成 SQL → asyncpg 执行
   - 参考: `core/query_service.py` 中的 `@REAL_CODE` 注释

2. **确认闭环补全**
   - 前端确认 → 后端 `ConfirmationService.resolve()` → 执行 SQL
   - 参考: `core/confirmation_service.py` + `middleware/confirmation_middleware.py`

3. **AgentResult 迁移**
   - 逐步将所有 `Agent.process()` 的 `dict` 返回值迁移到 `AgentResult`
   - 参考: `core/models.py` 中的 `AgentResult.to_dict()` 过渡方法

### Phase 2: 数据链路增强

4. **Graph → Report → PPT 数据传递**
   - 让后续 Agent 真正复用 Query Agent 的输出数据
   - 实现 `RouterTask.dependency_on` 的 DAG 调度

5. **S3/MinIO 对象存储对接**
   - 产物文件上传到 S3 → 返回预签名 URL
   - 参考: `core/models.py:Artifact` + `config.py StorageSettings`

### Phase 3: 质量与稳定性

6. **测试体系完善**
   - pytest fixtures 注入 mock 依赖
   - 补充集成测试（对接真实服务）

7. **意图识别升级**
   - 写操作检测: 关键词 → LLM 语义判断
   - 澄清判断: 长度检测 → LLM 信息完整度分析

8. **分布式支持**
   - 确认状态: 内存 dict → Redis
   - 会话管理: 内存 → PostgreSQL / Redis

---

> 📄 本系统基于四份设计文档构建：`设计手册.docx` / `agent设计手册.docx` / `中断式确认机制调研.docx` / `matplotlib.docx`  
> 🔧 如有疑问或需要扩展，请参考设计文档中的详细方案描述。  
> 🏷️ v2 增强来自 `multi_agent_system_2.0/` — 所有增强点均已标注 `@REAL_CODE` 注释。
